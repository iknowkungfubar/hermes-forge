...
class WorkflowRunner:
    ...
    async def _handle_tool_calls(
        ...
        if step_check.needs_nudge:
            messages.append(_nudge("step_enforcement", step_check.nudge.content if step_check.nudge else ""))
            return None

        # 3. Prerequisite check
        prereq_check = step_enforcer.check_prerequisites(validated_calls)
        if prereq_check.needs_nudge:
            messages.append(_nudge("prerequisite_skip", prereq_check.nudge.content if prereq_check.nudge else ""))
            return None
